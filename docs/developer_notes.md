...
`app/core/model_selection.py` only aggregates already-produced shape-model envelopes. It does not rerun fits. The ranking order is coverage (descending), median AICc/BIC rank, residual-pass rate, bound-hit rate, and uncertainty. **Main-model eligibility** requires all of: coverage ≥ `0.70`, residual pass rate ≥ `0.70`, bound-hit rate ≤ `0.30`, finite uncertainty (median relative stderr) ≤ `1.0`, and reliability pass rate ≥ `0.70` (labels not `invalid`/`low`, and score ≥ `0.50` when present). Failures are listed in `eligibility_failures`. Models that fail any gate remain in `AutoBatchRun.rankings` with `eligible_for_main_model=False` and cannot become `AutoBatchRun.main_model` (for example a fully converged model with `residual_pass_rate=0` is never the main model). The selected main model is fixed for the entire completed batch. `transition_flags` records an alternative per-frame winner only after it remains best for three consecutive frames; the flag is a review cue and never changes the main model automatically.
...
The user-facing Ti15 result package is intentionally compact after workbook export. Its root contains only `final_report_zh.md`, `final_results.csv`, `summary_tables.xlsx`, the two independent archives, and `README.md`; secondary CSV/JSON files, figures, and the `audit/`, `details/`, and `summary/` directories are moved under `review/`. The audit archive also includes a copy of the generated figures so a review can be performed without opening the primary result directory. Re-running the workbook builder supports both the compact layout and the pre-compaction layout.

The final Ti15 package was regenerated and replaced at `results/17_Ti15_300_2_iso_first10_model_free_20260712_122352/` using the effective interval. Validation recorded 10 curves, 2,140 imported points from 55,000 raw points, 214 points per frame, zero out-of-range q rows in the detail archive, 10/10 unchanged source hashes, and zero workbook formula errors. The complete test suite passed 564 tests.

## 2026-07-12 - Horizontal Parameter Tables In The Summary Workbook
