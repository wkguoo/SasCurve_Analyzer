...
ORIGIN_LONG_COLUMN_GUIDE = [
    ...
    (
        "q",
        "散射矢量模长",
        "横坐标，单位见 q_unit。q 越小对应越大的结构尺度，q 越大对应越小的结构尺度；常用近似尺度 d = 2*pi/q。",
        "常作为 X 轴。log-log 图看幂律区，Guinier 图看低 q 尺度，峰位 q* 可换算特征间距 d = 2*pi/q*。",
    ),
    (
        "I",
        "散射强度 I(q)",
...
def build_origin_long_guide_markdown(csv_filename: str = "curves_long.csv") -> str:
    ...
    lines.extend(
        [
            ...
            "3. Guinier 图: 低 q 区用 `q^2` 作 X 轴、`ln(I)` 作 Y 轴。若出现近似直线，可用于估计 Rg；只应在满足 qRg 经验范围且残差合理时解释。",
            "4. 峰位/特征间距图: 找到峰位 `q*` 后，用 `d = 2*pi/q*` 估计实空间特征间距。峰位移动通常表示平均距离变化，峰变宽通常表示有序度或尺寸分布变化。",
